          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Product Type List</h3>
               <a style="color: #FFF;" href="producttype_add.php"><div class="btn btn-info " style="margin: 5px 30px 5px;">
                     Add
                  </div></a>
              <div class="box-tools">
...
<script type="text/javascript">
    ...
    function pageselectCallback(page_index, jq, event_type){
     ...
     function loadData(start,per_page,event_type){
        ajaxloader.loadURL('ajax/product_type.php', {start:start,search:$('#search').val()}, function(res){                
                  var tr = '';
                 if(res.error=='1')
                 {
                 ...
                 else
                 {
                         ...
                         var result = res['rst'];                         
                          $.each(result, function(index, item){
                                  tr += "<tr>\
                                             <td>"+item.name+"</td>\
                                             <td>"+item.product_uniform+"</td>\
                                             <td><a class='fa fa-fw fa-edit' href='producttype_edit.php?id="+item.id+"''></a>&nbsp;<a type='submit' class='glyphicon glyphicon-trash' data-toggle='modal' onclick=destroy("+item.id+") ></a></td>\
                                          </tr>";
                          });
                  }
