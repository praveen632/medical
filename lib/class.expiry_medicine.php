...
class Expiry{
	...
	public function getTax($select = '*', $start_limit=-1){
       ...
       if($start_limit>-1){
           ...
           $results = $this->db->where(['stock_status'=>'1'])->order_by('id', 'ASC')->get($this->table, $select);
          }
            if($results){
                return $results;
            }
    ...
    public function getTaxBySearch($search_value, $select = '*', $start_limit=-1){
             if($start_limit>-1){                  
               ...
               $results = $this->db->where(['1'=>'1'])->_like(['name'=>$search_value])->order_by('id', 'ASC')->get($this->table1, $select);
              }
                if($results){
                    return $results;
                }
